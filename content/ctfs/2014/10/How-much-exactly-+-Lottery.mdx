...
just do some search on the internet, and we'll find this
[link](https://archive.org/stream/Untangling_the_Web/Untangling_the_Web_djvu.txt)
...
if we url-decode `%3D` the result will be `=`, so the cookie actually contains a base64 encode string. After we decode
the whole string, it shows `1496:1415db70fe9ddb119e23e9b2808cde38`.

After doing some research we found that `1415db70fe9ddb119e23e9b2808cde38` is actually `md5(1496)`. So the challenge is
quite simple: we just need to modify the cookie value to `base64_encode(1234567890:md5(1234567890))`, then we'll get the
flag.
